/**
 * Test script for the C/C++ Hello plugin
 */
...
async function main() {
  const pluginFile = path.join(__dirname, 'hello-plugin.wasm');
  
  console.log('=== Testing C/C++ Hello Plugin ===\n');
  
  // Check if the WASM file exists
  if (!fs.existsSync(pluginFile)) {
    console.warn('Plugin file not found:', pluginFile);
    console.warn('Please build the plugin first with: cd src/c-pdk && make');
